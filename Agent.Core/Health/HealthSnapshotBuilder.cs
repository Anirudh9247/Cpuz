...
public class HealthSnapshotBuilder : IHealthSnapshotBuilder
{
        ...
        StorageMetrics? storage)
    {
        ...
        if (hardware != null)
        {
            ...
            {
                snapshot.Cpu.TempC = SensorReading<double>.FromValue(hardware.CpuTemp.Value!, hardware.CpuTemp.Source, hardware.CpuTemp.IsFallback);
                if (hardware.CpuTemp.IsFallback) anyFallbackUsed = true;
                mainSource = hardware.CpuTemp.Source;
            }
            if (hardware.CpuUsage.HasValue)
            {
                snapshot.Cpu.LoadPercent = SensorReading<double>.FromValue(hardware.CpuUsage.Value!, hardware.CpuUsage.Source, hardware.CpuUsage.IsFallback);
            ...
            {
                snapshot.Gpu.TempC = SensorReading<double>.FromValue(hardware.GpuTemp.Value!, hardware.GpuTemp.Source, hardware.GpuTemp.IsFallback);
            ...
            {
                snapshot.Memory.UsagePercent = SensorReading<double>.FromValue(hardware.MemoryUsage.Value!, hardware.MemoryUsage.Source, hardware.MemoryUsage.IsFallback);
            }
